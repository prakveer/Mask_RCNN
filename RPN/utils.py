...
def pc_data_parser():
    ...
    #car_or_people = tf. where(iou_people>iou_car, tf.ones_like(iou_people), tf.zeros_like(iou_car))
    ...
    return im_tensor, binary_mask, ignore_mask, gt_regression, len(files)
    #return
...
def get_pc_dataset(batch_size):

    im_tensor, binary_mask, ignore_mask, gt_regression,  dataset_size = pc_data_parser()

    # Step 2: Create datasets and iterator
    data = tf.data.Dataset.from_tensor_slices((im_tensor, binary_mask, ignore_mask, gt_regression))
